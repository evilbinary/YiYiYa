...
Ret etk_bitmap_draw_rect(EtkBitmap *thiz, e32 x, e32 y, e32 width, e32 height,
                         u32 color) {
...
Ret etk_bitmap_draw_line_color(EtkBitmap *thiz, e32 x1, e32 y1, e32 x2, e32 y2,
                               u32 color) {
  int t;
  ...
  for (t = 0; t <= distance + 1; t++) {            //�������
    etk_bitmap_draw_pixel(thiz, row, col, color);  //����
    xerr += delta_x;
    yerr += delta_y;
    if (xerr > distance) {
      xerr -= distance;
...
Ret etk_bitmap_draw_line(EtkBitmap *thiz, e32 x1, e32 y1, e32 x2, e32 y2) {
  etk_bitmap_draw_line_color(thiz, x1, y1, x2, y2, GREEN);
  return RET_OK;
}