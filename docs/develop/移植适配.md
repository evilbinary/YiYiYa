...
3、移植先不移植mmu，运行正常后，再VM_ENABLE移植mmu，这样降低难度
...
通用寄存器，The general purpose registers, R0-R30


When executing at EL0, the PE uses the EL0 stack pointer, SP_EL0.
When executing at EL1, the PE uses the EL1 stack pointer, SP_EL1.
When executing at EL2, the PE uses the EL2 stack pointer, SP_EL2.
When executing at EL3, the PE uses the EL3 stack pointer, SP_EL3.

状态寄存器：The Saved Program Status Registers (SPSRs) are used to save PE state on taking exceptions.
EL1异常时候存放到SPSR_EL1


![](image/2026-02-22-00-52-51.png)




在 ARMv8-A 中，页表描述符格式与 ARMv7-A 有很大不同（VMSAv8-64）。请重点检查以下函数：

  1. 页描述符属性 (Attributes)

  ARMv8-A 的页描述符位定义如下：
  • AF (Access Flag, bit 10): 必须设置为 1，否则会触发 Access Flag Fault。
  • SH (Shareability, bits 9:8): 通常设为 0b11 (Inner Shareable)。
  • AP (Access Permissions, bits 7:6):
      • 00: EL1 Read/Write, EL0 None
      • 01: EL1 Read/Write, EL0 Read/Write (用户态访问需要这个)
      • 10: EL1 Read Only, EL0 None
      • 11: EL1 Read Only, EL0 Read Only
  • AttrIndx (bits 4:2): 对应 MAIR_EL1 寄存器中的内存类型（Normal, Device 等）
...
call4 Rotate Window by 4
  a4  return address
  callee’s a0
  etnry rotate window anjust stack


l32i  L32R—Load 32-bit PC-Relative
