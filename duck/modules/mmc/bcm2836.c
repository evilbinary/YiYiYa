...
static const uint32_t sd_acommands[]
    __attribute__((aligned(4))) = {SD_CMD_RESERVED(0),
                                   SD_CMD_RESERVED(1),
                                   SD_CMD_RESERVED(2),
                                   SD_CMD_RESERVED(3),
                                   SD_CMD_RESERVED(4),
...
static uint32_t emmc_get_version(void) {
  ...
  uint32_t sdversion = (ver & BCM2835_EMMC_SLOTISR_VER_SDVERSION_MASK) >>
                       BCM2835_EMMC_SLOTISR_VER_SDVERSION_SHIFT;
#ifdef EMMC_DEBUG
  uint32_t vendor = (ver & BCM2835_EMMC_SLOTISR_VER_VENDOR_MASK) >>
                    BCM2835_EMMC_SLOTISR_VER_VENDOR_SHIFT;
  uint32_t slot_status = ver & BCM2835_EMMC_SLOTISR_VER_SLOT_STATUS_MASK;
  EMMC_TRACE("vendor %x, sdversion %x, slot_status %x", (uint8_t)vendor,
             (uint8_t)sdversion, (uint8_t)slot_status);
#endif
...
static uint32_t emmc_get_clock_divider(const uint32_t target_clock) {
  ...
  EMMC_TRACE("target_clock: %ld, divisor: %x, actual_clock: %d, ret: %x0",
             target_clock, divisor, actual_clock, (int)ret);
#endif
...
static uint32_t emmc_set_clock(const uint32_t target_clock) {
  ...
  if (divider == SD_GET_CLOCK_DIVIDER_FAIL) {
    EMMC_TRACE("Couldn't get a valid divider for target clock %d Hz",
               target_clock);
    return -1;
  }

  // Wait for the command inhibit (CMD and DAT) bits to clear
  TIMEOUT_WAIT(!(BCM2835_EMMC->STATUS & (BCM2835_EMMC_STATUS_CMD_INHIBIT ||
                                         BCM2835_EMMC_STATUS_DATA_INHIBIT)),
               1000000);

  if ((BCM2835_EMMC->STATUS & (BCM2835_EMMC_STATUS_CMD_INHIBIT ||
                               BCM2835_EMMC_STATUS_DATA_INHIBIT)) != 0) {
    EMMC_TRACE("Timeout waiting for inhibit flags. Status %08x",
               BCM2835_EMMC->STATUS);
    return -1;
  }
  ...
  TIMEOUT_WAIT(BCM2835_EMMC->CONTROL1 & BCM2835_EMMC_CONTROL1_CLK_STABLE,
               1000000);
...
static uint32_t emmc_reset(void) {
  ...
  TIMEOUT_WAIT((BCM2835_EMMC->CONTROL1 & BCM2835_EMMC_CONTROL1_SRST_HC) == 0,
               1000000);
  ...
  EMMC_TRACE("control0: %08x, control1: %08x, control2: %08x",
...
static int emmc_reset_cmd(void) {
  ...
  TIMEOUT_WAIT((BCM2835_EMMC->CONTROL1 & BCM2835_EMMC_CONTROL1_RESET_CMD) == 0,
               1000000);
...
static int emmc_reset_dat(void) {
  ...
  TIMEOUT_WAIT((BCM2835_EMMC->CONTROL1 & BCM2835_EMMC_CONTROL1_RESET_DATA) == 0,
               1000000);
 ...
 * @param timeout
 */
static void emmc_issue_command(uint32_t cmd_reg, uint32_t argument,
                               uint32_t timeout) {
  ...
  if ((((cmd_reg & SD_CMD_RSPNS_TYPE_MASK) == SD_CMD_RSPNS_TYPE_48B) ||
    ...
    if ((BCM2835_EMMC->STATUS & BCM2835_EMMC_STATUS_DATA_INHIBIT) == 0) {
      ...
      if (((irpts & 0xffff0002) != 0x2) && ((irpts & 0xffff0002) != 0x100002)) {
        EMMC_TRACE(
            "Error occurred whilst waiting for transfer complete interrupt");
        dev->last_error = irpts & 0xffff0000;
        dev->last_interrupt = irpts;
        return;
      }
 ...
 * @ingroup EMMC
 *
 * Please note that the EMMC module restricts the maximum block size to the size
 * of the internal data FIFO which is 1k bytes.
 *
 * @param block_size
 */
 ...
 * @param timeout
 */
static void sd_issue_command(uint32_t command, uint32_t argument,
                             uint32_t timeout) {
  struct emmc_block_dev *dev = &block_dev;
...
static int sd_card_sanity_check(void) {
  if (sizeof(sd_commands) != (64 * sizeof(uint32_t))) {
    SD_TRACE("Fatal error, sd_commands of incorrect size: %d expected %d",
             sizeof(sd_commands), 64 * sizeof(uint32_t));
    return -1;
  }

  if (sizeof(sd_acommands) != (64 * sizeof(uint32_t))) {
    SD_TRACE("Fatal error, sd_acommands of incorrect size: %d expected %d",
 ...
 * @return
 */
static int sd_do_data_command(int is_write, uint8_t *buf, size_t buf_size,
                              uint32_t block_no) {
  ...
  if (!edev->card_supports_sdhc) {
    block_no *= BYTE_PER_SECTOR;
  }

  // This is as per HCSS 3.7.2.1
  if (buf_size < edev->block_size) {
    SD_TRACE("buffer size (%d) is less than block size (%d)", buf_size,
             edev->block_size);
    return -1;
  }
  ...
  while (retry_count < max_retries) {
    ...
    if (SUCCESS(edev)) {
      ...
      SD_TRACE("Error sending CMD%d, edev->last_error = %08x", command,
               edev->last_error);
      retry_count++;
...
int sd_card_init(void) {
  ...
  kmemset(ret, 0, sizeof(struct emmc_block_dev));
  ...
  SD_TRACE(
      "Note a timeout error on the following command (CMD8) is normal and "
      "expected if the SD card version is less than 2.0");
  ...
  SD_TRACE(
      "Note that a timeout error on the following command (CMD5) is normal and "
      "expected if the card is not a SDIO card");
  ...
  if (!TIMEOUT(ret)) {
    if (CMD_TIMEOUT(ret)) {
      ...
      SD_TRACE(
          "SDIO card detected - not currently supported, CMD5 returned %08x",
          ret->last_r0);
      return SD_ERROR;
    }
  }
  ...
  while (card_is_busy) {
    ...
    if (v2_later) {
      ...
      // Set 1.8v support
#ifdef SD_1_8V_SUPPORT
      if (!ret->failed_voltage_switch) v2_flags |= (1 << 24);
#endif
  ...
  SD_TRACE("Card CID: %08x%08x%08x%08x", card_cid_3, card_cid_2, card_cid_1,
           card_cid_0);
#endif
  ...
  if (!ret->card_supports_sdhc) {
    ...
    SD_TRACE("CMD16 response: %08x, Statud %d", ret->last_r0,
             (ret->last_r0 >> 9) & 0xf);
    if (FAIL(ret)) {
  ...
  SD_TRACE("ACMD51 response: %08x, Status %d", ret->last_r0,
           (ret->last_r0 >> 9) & 0xf);
  ...
  SD_TRACE("SCR: %08x%08x", __builtin_bswap32(ret->scr->scr[0]),
