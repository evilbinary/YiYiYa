...
Ret etk_platform_event_poll(EtkEvent *event) {
  ...
  screen_fill_rect(e.mouse.x, e.mouse.y, 4, 4, 0x00ff00);
  switch (e.type) {
    case MOUSE_MOTION:
		// printf(".");
...
__inline void put_pixel(screen_info_t *surface, int x, int y, u32 pixel) {
	...
	u8 *p = (u8 *) surface->buffer + y * surface->width + x * bpp;
	if (surface->height <= y || surface->width <= x||x<0||y<0)
		return;
	switch (bpp) {
	case 1:
		*p = pixel;
...
void put_pixel16(screen_info_t *surface, int x, int y, u16 pixel) {
	...
	//Set the pixel
	if (surface->height <= y || surface->width <= x)
		return;
...
void draw_rectangle(screen_info_t *surface, int x, int y, int width, int height, u32 color) {
...
void draw_dot_rectangle(screen_info_t *surface, int x, int y, int width, int height, u32 color) {
...
void fill_rectangle(screen_info_t *surface, int x, int y, int width, int height, u32 color) {
	int z = y + height;
	for (; y <= z; y++) {
		draw_hline(surface, x, y, width, color);
	}
